...
class ProgressCallbackManager:
    ...
    async def update_progress(self, current_bytes: int):
        ...
        # Calculate speed based on delta since last update
...
async def start_handler(client: Client, message: Message):
    welcome_text = (
        ...
        f"🌐 **Web Interface:** http://{FLASK_HOST}:{FLASK_PORT}\n\n"
...
async def media_handler(client: Client, message: Message):
    ...
    # Check file size limit (Telegram bot limit is 2GB)
    if file_size > 2 * 1024 * 1024 * 1024:
        await message.reply_text("❌ File size exceeds 2GB limit. Telegram bots cannot download files larger than 2GB.")
        return
    ...
    try:
        ...
        # Also set content disposition for better download naming
        ...
        # Add timeout for upload (30 minutes for 2GB file)
        ...
        # Generate URL with timeout (30 seconds)
        ...
        success_message = (
            ...
            f"🌐 **Manage in Web UI:** http://{FLASK_HOST}:{FLASK_PORT}"
        )

        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📥 Download File", url=presigned_url)],
            [InlineKeyboardButton("🌐 Open Web Interface", url=f"http://{FLASK_HOST}:{FLASK_PORT}")]
        ])
